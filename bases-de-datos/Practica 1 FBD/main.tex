...
\\
\textit{Para el siguente paso es agregar el comando en la terminal para poder agregar el repositorio este comando es: \\ "sudo sh -c ’echo "deb http://apt.postgresql.org/pub/repos/apt/ DEB\_VERSION-pgdg main" >> /etc/apt/sources.list.d/pgdg.list"}
...
\textit{Para comprobar finalmente que fue instalado correctamente y un paso de seguridad vamos a correr este primer comando "service postgresql status" este nos muestra que esta "down" porque no lo hemos empezado.\\
finalmente solamente establecemos contraseña con el siguiente comando "ALTER USER postgres WITH PASSWORD ’my\_password’;"}\\
\begin{itemize}
